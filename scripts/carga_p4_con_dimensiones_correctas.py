#!/usr/bin/env python3
"""
Script CORREGIDO para carga P4 con DIMENSIONES ESPECÍFICAS
SOLUCIÓN: Usar campos cube_side_cm, beam_width_cm, etc. para distinguir tamaños
"""
...
def analizar_tipo_muestra_con_dimensiones(tipo_original):
    """
    Analiza tipo de muestra y retorna tipo + dimensiones específicas
    SOLUCIÓN: Usar campos de dimensión específicos en BD
    """
    if not tipo_original:
        return {"tipo": "CUBO", "cube_side_cm": 10, "beam_width_cm": None, "beam_height_cm": None, "beam_span_cm": None}
    
    tipo_limpio = tipo_original.strip().upper()
    
    if "CUBO 10 X 10" in tipo_limpio:
        return {
            "tipo": "CUBO",
            "cube_side_cm": 10,
            "beam_width_cm": None,
            "beam_height_cm": None,
            "beam_span_cm": None,
            "descripcion": "CUBO 10x10 cm"
        }
    elif "CUBO 15 X 15" in tipo_limpio:
        return {
            "tipo": "CUBO", 
            "cube_side_cm": 15,
            "beam_width_cm": None,
            "beam_height_cm": None,
            "beam_span_cm": None,
            "descripcion": "CUBO 15x15 cm"
        }
    elif "VIGA" in tipo_limpio:
        return {
            "tipo": "VIGA",
            "cube_side_cm": None,
            "beam_width_cm": 15,  # Dimensiones estándar para vigas
            "beam_height_cm": 15,
            "beam_span_cm": 60,
            "descripcion": "VIGA 15x15x60 cm"
        }
        ...
        return {
            ...
            "beam_height_cm": None,
            "beam_span_cm": None,
            "descripcion": "CUBO 10x10 cm (default)"
        }

def procesar_csv_p4_con_dimensiones():
    """
    Procesa el CSV de P4 con DIMENSIONES ESPECÍFICAS CORRECTAS
    """
    ...
    # Contadores por tipo específico
    cubos_10x10 = 0
    cubos_15x15 = 0
    vigas = 0
    ...
    print("🚀 PROCESANDO CSV PLANTA 4 - CON DIMENSIONES ESPECÍFICAS")
    print("=" * 70)
        ...
        with open(archivo_csv, 'r', encoding='utf-8') as file:
            ...
            for row_num, row in enumerate(reader, start=2):
                ...
                try:
                    ...
                    # 6. DETERMINAR TIPO DE EDAD
                    edad_1 = edades[0]
                    es_horas = edad_1 is not None and edad_1 > 10
                    
                    print(f"📋 Remisión {remision}: {cantidad_muestras} muestras, {'HORAS' if es_horas else 'DÍAS'} (EDAD1={edad_1})")
                    
                    # 7. CREAR MUESTRAS CON DIMENSIONES ESPECÍFICAS
                    for i in range(cantidad_muestras):
                        ...
                        if edad is not None and edad > 0:
                            ...
                            # ANALIZAR TIPO CON DIMENSIONES
                            info_muestra = analizar_tipo_muestra_con_dimensiones(tipo_original)
                            
                            # Contar tipos específicos
                            if info_muestra["tipo"] == "CUBO":
                                if info_muestra["cube_side_cm"] == 10:
                                    cubos_10x10 += 1
                                elif info_muestra["cube_side_cm"] == 15:
                                    cubos_15x15 += 1
                            elif info_muestra["tipo"] == "VIGA":
                                vigas += 1
                            
                            # 8. CALCULAR FECHA DE ENSAYO
                            if es_horas:
                            ...
                            # 10. CREAR DATOS DE MUESTRA CON DIMENSIONES
                            muestra_data = {
                                'remision': remision,
                                'identificacion': identificacion,
                                'tipo_muestra': info_muestra["tipo"],
                                'cube_side_cm': info_muestra["cube_side_cm"],
                                'beam_width_cm': info_muestra["beam_width_cm"],
                                'beam_height_cm': info_muestra["beam_height_cm"],
                                'beam_span_cm': info_muestra["beam_span_cm"],
                                'fecha_programada_ensayo': str(fecha_ensayo),
                                'fecha_programada_ensayo_ts': fecha_ensayo_ts,
                                'estado': estado,
                                'plant_id': PLANT_P4_ID,
                                'event_timezone': 'America/Mexico_City',
                                'descripcion': info_muestra["descripcion"]
                            }
                            
                            muestras_data.append(muestra_data)
                            muestras_creadas += 1
                            
                            print(f"   → {identificacion}: {info_muestra['descripcion']} - {fecha_ensayo} - {estado}")
    ...
    # GENERAR SQL CON DIMENSIONES
    print(f"\n📝 Generando SQL con dimensiones específicas...")
    generar_sql_con_dimensiones(muestreos_data, muestras_data, ensayos_data)
    
    # ESTADÍSTICAS FINALES
    print(f"\n📊 ESTADÍSTICAS CON DIMENSIONES:")
    print(f"  Total registros: {total_registros}")
    print(f"  Muestreos: {muestreos_creados}")
    print(f"  Muestras: {muestras_creadas}")
    print(f"  - Cubos 10x10: {cubos_10x10}")
    print(f"  - Cubos 15x15: {cubos_15x15}")
    print(f"  - Vigas: {vigas}")
...
def generar_sql_con_dimensiones(muestreos_data, muestras_data, ensayos_data):
    """Genera SQL con dimensiones específicas"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    archivo_sql = f"carga_p4_con_dimensiones_{timestamp}.sql"
    
    with open(archivo_sql, 'w', encoding='utf-8') as f:
        f.write("-- SCRIPT P4 CON DIMENSIONES ESPECÍFICAS CORRECTAS\n")
        f.write(f"-- Generado: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("-- SOLUCIÓN: cube_side_cm para distinguir 10x10 vs 15x15\n")
        f.write("-- CAMPOS: cube_side_cm, beam_width_cm, beam_height_cm, beam_span_cm\n\n")
        ...
        # MUESTRAS CON DIMENSIONES
        f.write("-- ==========================================\n")
        f.write("-- PASO 2: CREAR MUESTRAS CON DIMENSIONES ESPECÍFICAS\n")
        f.write("-- ==========================================\n\n")
        
        if muestras_data:
            f.write("INSERT INTO public.muestras (\n")
            f.write("  muestreo_id, identificacion, tipo_muestra, \n")
            f.write("  cube_side_cm, beam_width_cm, beam_height_cm, beam_span_cm,\n")
            f.write("  fecha_programada_ensayo, fecha_programada_ensayo_ts, estado, \n")
            ...
            f.write("  datos.cube_side_cm,\n")
            f.write("  datos.beam_width_cm,\n")
            f.write("  datos.beam_height_cm,\n")
            f.write("  datos.beam_span_cm,\n")
            ...
            for i, mu in enumerate(muestras_data):
                valores = [
                    ...
                    f"'{mu['identificacion']}'",
                    f"'{mu['tipo_muestra']}'",
                    str(mu['cube_side_cm']) if mu['cube_side_cm'] else "NULL",
                    str(mu['beam_width_cm']) if mu['beam_width_cm'] else "NULL",
                    str(mu['beam_height_cm']) if mu['beam_height_cm'] else "NULL",
                    str(mu['beam_span_cm']) if mu['beam_span_cm'] else "NULL",
                ...
                f.write(f"    ({', '.join(valores)}){separador}  -- {mu['descripcion']}\n")
            
            f.write(") AS datos(remision, identificacion, tipo_muestra, cube_side_cm, beam_width_cm, beam_height_cm, beam_span_cm, fecha_programada_ensayo, fecha_programada_ensayo_ts, estado, plant_id, event_timezone)\n")
        ...
        # VERIFICACIONES CON DIMENSIONES
        f.write("-- ==========================================\n")
        f.write("-- PASO 4: VERIFICACIONES CON DIMENSIONES\n")
        f.write("-- ==========================================\n\n")
        
        f.write("-- Verificar tipos con dimensiones específicas\n")
        f.write("SELECT \n")
        f.write("  'Muestras P4 por dimensión' as tabla,\n")
        f.write("  tipo_muestra,\n")
        f.write("  cube_side_cm,\n")
        f.write("  CASE \n")
        f.write("    WHEN tipo_muestra = 'CUBO' AND cube_side_cm = 10 THEN 'CUBO 10x10'\n")
        f.write("    WHEN tipo_muestra = 'CUBO' AND cube_side_cm = 15 THEN 'CUBO 15x15'\n")
        f.write("    WHEN tipo_muestra = 'VIGA' THEN 'VIGA'\n")
        f.write("    ELSE 'OTRO'\n")
        f.write("  END as tipo_detallado,\n")
        ...
        f.write("GROUP BY tipo_muestra, cube_side_cm\n")
        f.write("ORDER BY tipo_muestra, cube_side_cm;\n\n")
        
        f.write("-- FIN SCRIPT CON DIMENSIONES ESPECÍFICAS\n")
    
    print(f"✅ SQL con dimensiones generado: {archivo_sql}")

if __name__ == "__main__":
    procesar_csv_p4_con_dimensiones()
