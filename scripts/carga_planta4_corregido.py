#!/usr/bin/env python3
"""
Script CORREGIDO para carga masiva de muestreos y muestras de Planta 4
Basado en el archivo: archivoexcel/Calidad P4.csv

LÓGICA CORREGIDA:
- Solo crear las muestras según "Cantidad de Muestras" (3 o 4)
- EDAD1 → M1 → CARGA1 (solo si EDAD1 tiene valor)
- EDAD2 → M2 → CARGA2 (solo si EDAD2 tiene valor)  
- EDAD3 → M3 → CARGA3 (solo si EDAD3 tiene valor)
- EDAD4 → M4 → CARGA4 (solo si EDAD4 tiene valor)

REGLA ESPECIAL DE EDADES:
- Si EDAD1 > 10: todas las edades están en HORAS
- Si EDAD1 <= 10: todas las edades están en DÍAS

TIPOS DE MUESTRA CORRECTOS:
- "CUBO 10 X 10" → "CUBO"
- "CUBO 15 X 15" → "CUBO" 
- "VIGA" → "VIGA"
"""
...
def convertir_hora_formato_mixto(hora_str):
    """
    Convierte hora que puede estar en formato HH:MM o decimal
    """
    ...
    try:
        ...
        if ':' in hora_str:
            return hora_str + ":00" if len(hora_str.split(':')) == 2 else hora_str
...
def simplificar_tipo_muestra(tipo_original):
    """
    Simplifica el tipo de muestra CORRECTAMENTE
    """
...
def procesar_csv_planta4_corregido():
    """
    Procesa el archivo CSV de Planta 4 con lógica CORREGIDA
    """
    ...
    print("🔄 Procesando archivo CSV de Planta 4 (CORREGIDO)...")
        ...
        with open(archivo_csv, 'r', encoding='utf-8') as file:
            ...
            for row_num, row in enumerate(reader, start=2):
                ...
                try:
                    ...
                    # IMPORTANTE: Verificar cantidad de muestras
                    ...
                    if not cantidad_muestras or cantidad_muestras not in [3, 4]:
                        errores.append(f"Fila {row_num}: Cantidad de muestras inválida: {cantidad_muestras}")
                        continue
                    ...
                    # Procesar edades y crear muestras CORRECTAMENTE
                    edades = [
                    ...
                    if es_horas:
                        ...
                        print(f"  📅 Remisión {remision}: Edades en HORAS (EDAD1={edad_1}, cantidad={cantidad_muestras})")
                    else:
                        registros_con_dias += 1
                        print(f"  📅 Remisión {remision}: Edades en DÍAS (EDAD1={edad_1}, cantidad={cantidad_muestras})")
                    
                    # CREAR MUESTRAS SOLO SEGÚN CANTIDAD ESPECIFICADA
                    for i in range(cantidad_muestras):  # Solo crear las muestras necesarias
                        edad = edades[i] if i < len(edades) else None
                        
                        if edad is not None and edad > 0:
                            identificacion = f"M{i+1}"
                            tipo_muestra_original = tipos_muestra[i] if i < len(tipos_muestra) else "CUBO 10 X 10"
    ...
    print("📝 Generando script SQL corregido...")
    generar_sql_carga(muestreos_data, muestras_data, ensayos_data)
    
    # Mostrar estadísticas
    print("\n📊 ESTADÍSTICAS DE PROCESAMIENTO CORREGIDO")
...
def generar_sql_carga(muestreos_data, muestras_data, ensayos_data):
    """
    Genera el script SQL completo para la carga masiva CORREGIDA
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    archivo_sql = f"carga_planta4_corregido_{timestamp}.sql"
    
    with open(archivo_sql, 'w', encoding='utf-8') as f:
        f.write("-- Script de carga masiva CORREGIDO para Planta 4\n")
        f.write(f"-- Generado: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"-- Archivo fuente: archivoexcel/Calidad P4.csv\n")
        f.write(f"-- Plant ID: {PLANT_P4_ID}\n")
        f.write("-- LÓGICA CORREGIDA: Solo crear muestras según 'Cantidad de Muestras'\n")
        ...
        f.write("-- FIN DEL SCRIPT CORREGIDO\n")
    
    print(f"✅ Script SQL corregido generado: {archivo_sql}")

if __name__ == "__main__":
    print("🚀 Iniciando procesamiento CORREGIDO de carga masiva - Planta 4")
    print("=" * 70)
    
    procesar_csv_planta4_corregido()
    
    print("\n✅ Procesamiento corregido completado!")
    print("\nPróximos pasos:")
    print("1. Revisar el archivo SQL corregido")
