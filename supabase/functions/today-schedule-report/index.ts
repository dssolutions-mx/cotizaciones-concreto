...
/**
 * TODAY SCHEDULE REPORT EDGE FUNCTION - MULTI-PLANT VERSION
 * 
 * Same as daily-schedule-report but defaults to TODAY instead of tomorrow.
 * Sends personalized schedule reports for ALL plants.
...
function generateSummaryHtml(plantName: string, totals: PlantOrders['totals']) {
  return `
...
function generateEmailContent(recipientName: string, plantOrdersList: PlantOrders[], formattedDate: string) {
  ...
  const plantsContent = plantOrdersList.map(po => {
    ...
    const summaryHtml = generateSummaryHtml(po.plant.name, po.totals);
        ...
        <div style="max-width: 800px; margin: 0 auto; background-color: #FFFFFF; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
          ...
          ${totalOrders > 0 ? `
              <div style="background-color: #F0F9FF; padding: 15px; border-radius: 8px; margin-bottom: 30px; text-align: center;">
                ...
                <strong>Total: ${totalOrders} pedido${totalOrders !== 1 ? 's' : ''} programado${totalOrders !== 1 ? 's' : ''} para HOY</strong>
                </p>
...
serve(async (req) => {
  try {
    ...
    console.log(`Using target date: ${targetDateString} (Mexico time) - TODAY report`);
    ...
    console.log(`Found ${allOrders.length} total orders for TODAY ${targetDateString}`);
    ...
    for (const recipient of recipients as Recipient[]) {
      ...
      if (recipient.role === 'EXECUTIVE') {
        ...
        // Users with only business_unit_id - skip for now
        continue;
      }
      ...
      const emailContent = generateEmailContent(recipientName, plantOrdersList, formattedDate);
      const totalOrders = plantOrdersList.reduce((sum, po) => sum + po.orders.length, 0);
    ...
    const emailData = {
      personalizations: [{
          ...
          subject: `Programación de Entregas HOY - ${formattedDate} (${totalOrders} pedidos)`
      }],
      from: {
        email: FROM_EMAIL,
        name: FROM_NAME
