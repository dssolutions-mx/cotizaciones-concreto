...
serve(async (_req) => {
  try {
    ...
    const { error: refreshErr, data: refreshData } = await supabase.rpc('ema_refresh_compliance_and_programa', {
      p_instrumento_id: null,
    });
    if (refreshErr) {
      console.error('ema_refresh_compliance_and_programa failed:', refreshErr);
    } else {
      console.log('ema_refresh_compliance_and_programa:', refreshData);
    }
