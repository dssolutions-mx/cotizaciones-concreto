...
// Default expiration: 7 days (604800 seconds) - extended from 24h to accommodate weekends
const generateActionToken = async (orderId, action, recipientEmail, expiresIn = 604800) => {
  try {
...
serve(async (req)=>{
  ...
  for (const recipient of recipients) {
    ...
    if (!response.ok) {
      ...
      // Store tokens in database for validation using UPSERT to handle duplicates
      // (unique constraint on order_id + recipient_email)
      await supabase.from('credit_action_tokens').upsert({
        order_id: order.id,
        recipient_email: recipient.email,
        approve_token: approveToken,
        reject_token: rejectToken,
        jwt_token: approveToken,
        expires_at: new Date(Date.now() + 604800 * 1000).toISOString() // 7 days from now
      }, { onConflict: 'order_id,recipient_email' });
    }
  }
