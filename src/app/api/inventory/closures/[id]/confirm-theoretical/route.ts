...
/** Snapshot refresh + historical calc can exceed default serverless timeout. */
...
export async function POST(
  ...
  try {
    ...
    // Service role for delete/insert snapshot + status (RLS-safe, same as evidence upload).
    const admin = createAdminClientForApi();
    const service = new InventoryClosureService(admin);
    await service.confirmTheoreticalReview(closureId);

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('[POST confirm-theoretical]', error);
