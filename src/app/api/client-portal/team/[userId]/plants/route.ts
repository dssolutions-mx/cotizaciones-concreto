...
/**
 * PATCH /api/client-portal/team/[userId]/plants
 * `userId` = team member's auth user id (user_profiles.id).
 * Body: { plant_ids: string[] } — empty array = all plants for this membership.
 */
export async function PATCH(
  ...
  try {
    ...
    const { plant_ids, client_id: bodyClientId } = parsed.data;
    const clientIdParam =
    ...
    if (resolved.ctx.roleWithinClient !== 'executive') {
      return NextResponse.json(
        { error: 'Access denied. Only executives can update team plant scope.' },
        { status: 403 }
      );
    }
    ...
    if (plant_ids.length > 0) {
      const { count, error: cntErr } = await supabase
        ...
        .in('id', plant_ids);
      if (cntErr) {
        return NextResponse.json({ error: 'Failed to validate plants' }, { status: 500 });
      }
      if ((count ?? 0) !== plant_ids.length) {
        return NextResponse.json({ error: 'One or more plants are not valid' }, { status: 400 });
      }
    }
    ...
    const { error: plantErr } = await replaceClientPortalMembershipPlantIds(
      admin,
      membership.id,
      plant_ids.length > 0 ? plant_ids : []
    );

    if (plantErr) {
      console.error('team plants PATCH:', plantErr);
      return NextResponse.json({ error: 'Failed to update plant scope' }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (e) {
    console.error('team plants PATCH:', e);
