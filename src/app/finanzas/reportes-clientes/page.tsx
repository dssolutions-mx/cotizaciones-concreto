...
import { ReportColumnPicker } from '@/components/reports/ReportColumnPicker';
...
export default function ReportesClientesPage() {
  ...
  // ── Column management ─────────────────────────────────────────────────────
  const setColumnOrder = useCallback((cols: ReportColumn[]) => {
    setOrderedCols(cols);
  }, []);
  const removeCol = useCallback((id: string) => {
    const meta = AVAILABLE_COLUMNS.find((c) => c.id === id);
          ...
          <Card className="border-stone-200">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-xs text-stone-500">
                Selecciona columnas, filtra en la lista de disponibles y arrastra
                el ícono de agarre para ordenar. El orden se refleja en PDF y
                Excel.
              </CardDescription>
              {reportLoading && (
                <p
