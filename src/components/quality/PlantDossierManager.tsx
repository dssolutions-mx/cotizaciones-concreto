...
interface PlantDossierManagerProps {
  plantId: string;
  plantCode?: string;
}

export default function PlantDossierManager({ plantId, plantCode }: PlantDossierManagerProps) {
  ...
  const loadDossiers = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/plants/dossier?plant_id=${plantId}`);
      const result = await res.json();
      if (res.ok && result.success) {
        setRecords(result.data || []);
      } else {
        console.error('Error loading dossiers:', result.error);
      }
    } catch (e) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/plants/dossier', { method: 'POST', body: formData });
      const result = await res.json();
      if (res.ok && result.success) {
        toast.success(result.message || 'Dossier subido exitosamente');
        ...
        toast.error(result.error || 'Error al subir dossier');
      }
    } catch (e) {
      console.error('Error uploading dossier:', e);
      toast.error('Error al subir dossier');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDelete = async (id: string, fileName: string) => {
    if (!confirm(`¿Está seguro de eliminar el dossier "${fileName}"?`)) return;
    try {
      const res = await fetch(`/api/plants/dossier?id=${id}`, { method: 'DELETE' });
      const result = await res.json();
      if (res.ok && result.success) {
        toast.success(result.message || 'Dossier eliminado exitosamente');
        loadDossiers();
      } else {
        toast.error(result.error || 'Error al eliminar dossier');
      }
    } catch (e) {
      console.error('Error deleting dossier:', e);
      toast.error('Error al eliminar dossier');
    }
  };
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold !text-gray-900">Dossier de Calidad {plantCode ? `· ${plantCode}` : ''}</span>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="!text-gray-900">Subir Dossier de Calidad</DialogTitle>
              <DialogDescription className="!text-gray-700">El archivo debe ser PDF, máximo 20MB</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="dossier-file" className="!text-gray-900 font-semibold">Archivo PDF *</Label>
                <Input ref={fileInputRef} id="dossier-file" type="file" accept="application/pdf" onChange={handleFileSelect} className="!text-gray-900" />
                {selectedFile && (
              ...
              <div className="space-y-2">
                ...
                <Textarea id="notes" placeholder="Notas del dossier..." value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} className="!text-gray-900 placeholder:text-gray-500" />
              </div>
            </div>
            <DialogFooter>
              <Button
      ...
      {loading ? (
        ...
        <div className="text-center py-6 text-sm bg-white rounded-lg border-2 border-dashed border-gray-200">
          ...
          <p className="font-medium !text-gray-700">Sin dossier</p>
          <p className="text-xs !text-gray-600 mt-1">Sube el primer dossier de calidad</p>
        </div>
      ) : (
                ...
                <div className="flex flex-col gap-1 flex-shrink-0">
                  {rec.url ? (
                    <a href={rec.url} target="_blank" rel="noopener noreferrer">
                      <Button variant="secondary" size="sm" className="h-7 w-7 p-0 bg-white border border-gray-300 hover:bg-gray-50 !text-black" title="Ver dossier">
