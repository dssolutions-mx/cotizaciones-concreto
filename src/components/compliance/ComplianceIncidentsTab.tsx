...
function StatusBadge({ status }: { status: DisputeStatus }) {
  return (
    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[status] ?? 'bg-stone-100 text-stone-700'}`}>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-6 py-4">
          ...
          {dispute.recipients && (
            <div className="rounded-md border border-stone-200 bg-stone-50 p-3 text-xs space-y-1">
              <p><span className="font-medium text-stone-600">Para:</span> {dispute.recipients.to.join(', ') || '—'}</p>
              <p><span className="font-medium text-stone-600">CC:</span> {dispute.recipients.cc.join(', ') || '—'}</p>
            </div>
          )}
          ...
          <div className="space-y-3 rounded-md border border-stone-200 p-4">
            ...
            <div className="space-y-1">
              ...
              <Textarea
                ...
                placeholder="Agrega contexto, justificación o acciones tomadas..."
                rows={3}
              />
            </div>
          </div>
...
export function ComplianceIncidentsTab({ dateFrom, dateTo, plantCode }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (plantCode !== 'all') params.set('plantCode', plantCode);
      const res = await fetch(`/api/compliance/daily/disputes?${params.toString()}`);
      const json = await res.json();
      if (!res.ok) {
        toast.error(json.error || 'Error al cargar incidentes');
        return;
      }
    ...
    <div className="space-y-4">
      ...
      {loading && disputes.length === 0 ? (
        ...
        <div className="rounded-lg border border-dashed border-stone-200 py-12 text-center text-sm text-stone-400">
          {disputes.length === 0
            ? 'No se han enviado correos de compliance para este período.'
        ...
        <div className="overflow-x-auto rounded-lg border border-stone-200">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-200 bg-stone-50 text-left text-xs font-medium text-stone-500">
                ...
                <th className="px-4 py-3">Resuelto el</th>
              ...
              {visible.map((d) => (
                <tr key={d.id} className="hover:bg-stone-50/60">
                  ...
                  <td className="px-4 py-3 text-xs text-stone-500 whitespace-nowrap">
                    {new Date(d.sent_at).toLocaleDateString('es-MX')}
                  </td>
                  <td className="px-4 py-3 text-xs text-stone-500 whitespace-nowrap">
